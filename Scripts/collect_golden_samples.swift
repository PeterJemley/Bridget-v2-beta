...
class GoldenSampleCollector {
  ...
  private func collectWeekendSample() async throws {
    ...
    print(
      "   📝 One-liner: Sunday, \(formatDate(lastSunday)) - Weekend traffic patterns with reduced volume, 3 bridges, complete 24-hour coverage, \(sampleData.count) records"
    )
  }
  ...
  private func collectDSTBoundarySample() async throws {
    ...
    print("   📊 Records: \(sampleData.count)")
    print(
      "   📝 One-liner: \(formatDate(dstDate)) - Daylight Saving Time transition day with timezone handling, 3 bridges, complete 24-hour coverage, \(sampleData.count) records"
    )
  }
  ...
  private func createSampleRecord(timestamp: Date, bridgeId: Int, trafficMultiplier: Double)
    -> [String: Any]
  {
    let calendar = Calendar.current
  ...
  private func getSampleDescription(for sampleType: String) -> String {
    ...
    case "Weekend Pattern":
      return
        "Weekend traffic patterns with reduced volume, different timing patterns, lower rush hour intensity"
    case "DST Boundary":
      return
        "Daylight Saving Time transition day with timezone handling, potential data anomalies, normal weekday patterns"
    default:
