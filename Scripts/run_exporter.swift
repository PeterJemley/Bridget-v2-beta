...
class SimpleBridgeDataExporter {
  ...
  func exportToNDJSON(ticks: [SimpleProbeTick], to url: URL) throws {
    ...
    for tick in ticks {
      ...
      if crossK < 0 { crossK = 0
        hadCorrection = true
      }
      if crossN < crossK { crossN = crossK
        hadCorrection = true
      }
      if viaPenalty < 0 { viaPenalty = 0
        hadCorrection = true
      } else if viaPenalty > 900 { viaPenalty = 900
        hadCorrection = true
      }
      if gateAnom < 1.0 { gateAnom = 1.0
        hadCorrection = true
      } else if gateAnom > 8.0 { gateAnom = 8.0
        hadCorrection = true
      }
      if alternatesTotal < 1 { alternatesTotal = 1
        hadCorrection = true
      }
      if alternatesAvoid < 0 { alternatesAvoid = 0
        hadCorrection = true
      } else if alternatesAvoid > alternatesTotal { alternatesAvoid = alternatesTotal
        hadCorrection = true
      }
