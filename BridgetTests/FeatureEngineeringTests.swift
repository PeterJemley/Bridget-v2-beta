...
struct FeatureEngineeringGoldenTests {
  ...
  func goldenSampleFeatures() async throws {
    let horizons = [0, 3]
    let result = try generateFeatures(
      ticks: Self.goldenTicks, horizons: horizons, deterministicSeed: 42)
...
struct FeatureEngineeringValidationHelperTests {
  ...
  func testValidateFeatureVector() async throws {
    ...
    let validFeatures = try generateFeatures(
      ticks: FeatureEngineeringGoldenTests.goldenTicks, horizons: [0])
    let validFeatureVector = validFeatures[0][0]

    // When/Then: Valid feature vector should pass validation
    #expect(
      validateFeatureVector(validFeatureVector), "Valid feature vector should pass validation")
...
struct FeatureEngineeringValidationTests {
  ...
  func deterministicResults() async throws {
    ...
    let result1 = try generateFeatures(
      ticks: FeatureEngineeringGoldenTests.goldenTicks, horizons: [0], deterministicSeed: seed)
    let result2 = try generateFeatures(
      ticks: FeatureEngineeringGoldenTests.goldenTicks, horizons: [0], deterministicSeed: seed)
  ...
  func differentResultsWithDifferentSeeds() async throws {
    ...
    let result1 = try generateFeatures(
      ticks: FeatureEngineeringGoldenTests.goldenTicks, horizons: [0], deterministicSeed: seed1)
    let result2 = try generateFeatures(
      ticks: FeatureEngineeringGoldenTests.goldenTicks, horizons: [0], deterministicSeed: seed2)
  ...
  func deterministicBehaviorWithLargerDatasets() async throws {
    ...
    let result1 = try generateFeatures(
      ticks: largerDataset, horizons: [0, 3, 6], deterministicSeed: 42)
    let result2 = try generateFeatures(
      ticks: largerDataset, horizons: [0, 3, 6], deterministicSeed: 42)
    let result3 = try generateFeatures(
      ticks: largerDataset, horizons: [0, 3, 6], deterministicSeed: 999)
    ...
    for (horizon1, horizon2) in zip(result1, result2) {
      #expect(horizon1.count == horizon2.count)
      for (fv1, fv2) in zip(horizon1, horizon2) {
        #expect(fv1.min_sin == fv2.min_sin, "min_sin should be identical")
        #expect(fv1.min_cos == fv2.min_cos, "min_cos should be identical")
        #expect(fv1.open_5m == fv2.open_5m, "open_5m should be identical")
    ...
    for (horizon1, horizon3) in zip(result1, result3) {
      ...
      for (fv1, fv3) in zip(horizon1, horizon3) {
        #expect(fv1.min_sin == fv3.min_sin, "min_sin should be identical regardless of seed")
        #expect(fv1.min_cos == fv3.min_cos, "min_cos should be identical regardless of seed")
      }
    }
  }
  ...
  func handlesEdgeCasesInLargerDatasets() async throws {
    ...
    let result2 = try generateFeatures(
      ticks: edgeCaseDataset.reversed(), horizons: [0], deterministicSeed: 1)
    let result3 = try generateFeatures(
      ticks: edgeCaseDataset, horizons: [0], deterministicSeed: 999)
    ...
    #expect(
      totalFeatures1 == totalFeatures2,
      "Feature count should be identical regardless of input order")
    #expect(
      totalFeatures1 == totalFeatures3, "Feature count should be identical regardless of seed")
  }
