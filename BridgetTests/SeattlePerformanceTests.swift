...
struct SeattlePerformanceTests {
  ...
  func fixtureIntegrity() throws {
    ...
    #expect(result.nodeCount == 4,
            "Expected 4 nodes in Phase 1 fixture, got \(result.nodeCount)")
    #expect(result.edgeCount == 4,
            "Expected 4 edges in Phase 1 fixture, got \(result.edgeCount)")
    #expect(result.bridgeCount == 2,
            "Expected 2 bridges in Phase 1 fixture, got \(result.bridgeCount)")
  ...
  func goldenPathComparison() throws {
    ...
    let found = try testHarness.pathEnumerationService.enumeratePaths(from: "A",
                                                                      to: "C",
                                                                      in: testHarness.fixtureGraph)

    let comparison = testHarness.pathEnumerationService
      .compareWithGoldenPaths(found: found,
                              expected: fixture.expectedPaths)
    #expect(comparison.isSuccess,
            "Golden comparison failed: \(comparison.description)")
  }

  @Test("Shortest path correctness: A -> C")
  func shortestPathCorrectness() throws {
    let shortest = try testHarness.pathEnumerationService.shortestPath(from: "A",
                                                                       to: "C",
                                                                       in: testHarness.fixtureGraph)
    ...
    #expect(shortest?.totalTravelTime == 500,
            "Shortest path should be 500s in fixture")
  ...
  func noPathPairs() throws {
    ...
    for (start, end) in pairs {
      let paths = try testHarness.pathEnumerationService.enumeratePaths(from: start,
                                                                        to: end,
                                                                        in: testHarness.fixtureGraph)
      #expect(paths.isEmpty, "Expected no paths from \(start) to \(end)")
      let shortest = try testHarness.pathEnumerationService.shortestPath(from: start,
                                                                         to: end,
                                                                         in: testHarness.fixtureGraph)
      #expect(shortest == nil,
              "Expected no shortest path from \(start) to \(end)")
    }
  }
  ...
  func algorithmParityDFSvsYen() throws {
    ...
    let dfsPaths = try dfsEnum.enumeratePaths(from: "A",
                                              to: "C",
                                              in: testHarness.fixtureGraph)
    let yensPaths = try yensEnum.enumeratePaths(from: "A",
                                                to: "C",
                                                in: testHarness.fixtureGraph)
    ...
    #expect(dfsSet == yensSet,
            "DFS and Yen's should return the same set of paths for K=2 on the fixture")
  }

  @Test(
    "Pruning behavior: Tight maxTimeOverShortest leaves only the shortest path"
  )
  func pruningWithTightMaxTimeOverShortest() throws {
    ...
    let prunedPaths = try prunedEnum.enumeratePaths(from: "A",
                                                    to: "C",
                                                    in: testHarness.fixtureGraph)
    #expect(prunedPaths.count == 1,
            "Expected only the shortest path to remain with tight pruning")
  ...
  func originDestinationPathEnumeration() throws {
    ...
    for (start, end, maxPaths, expectedCount) in testCases {
      do {
        let paths = try testHarness.pathEnumerationService
          .enumeratePaths(from: start,
                          to: end,
                          in: testHarness.fixtureGraph)
        ...
        Issue.record(
          "Enumeration failed for \(start) -> \(end): \(error)"
        )
  ...
  func algorithmComparison() throws {
    ...
    for k in kValues {
      do {
        ...
        let dfsPaths = try enumSvc.enumeratePaths(from: origin,
                                                  to: destination,
                                                  in: testHarness.fixtureGraph)
  ...
  @Test(
    "Cache performance (feature cache only): second run faster and hit rate improves"
  )
  func cachePerformance_featureCacheOnly() async throws {
    ...
    let warmupPaths = try enumService.enumeratePaths(from: "A",
                                                     to: "C",
                                                     in: testHarness.fixtureGraph)
    ...
    let paths1 = try enumService.enumeratePaths(from: "A",
                                                to: "C",
                                                in: testHarness.fixtureGraph)
    let scores1 = try await scoringService.scorePaths(paths1,
                                                      departureTime: fixedDeparture)
    let run1 = CFAbsoluteTimeGetCurrent() - t1
    #expect(!scores1.isEmpty)

    // Capture stats after run 1
    let (hitsAfter1, missesAfter1, hitRateAfter1) =
      scoringService.getCacheStatistics()
    print(
    ...
    let paths2 = try enumService.enumeratePaths(from: "A",
                                                to: "C",
                                                in: testHarness.fixtureGraph)
    let scores2 = try await scoringService.scorePaths(paths2,
                                                      departureTime: fixedDeparture)
    ...
    let (hitsAfter2, missesAfter2, hitRateAfter2) =
      scoringService.getCacheStatistics()
    print(
    ...
    #expect(missesAfter2 >= missesAfter1,
            "Misses are cumulative counters; should not decrease")
    #expect(hitRateAfter2 >= hitRateAfter1,
  ...
  @Test(
    "Cache performance (path memoization only): hot enumeratePaths faster"
  )
  func cachePerformance_pathMemoizationOnly() throws {
    ...
    _ = try? enumService.enumeratePaths(from: "A",
                                        to: "B",
                                        in: testHarness.fixtureGraph)
  ...
  @Test(
    "Memory stability across repeated enumeration + scoring (Phase 1 fixture)"
  )
  func memoryStability() async throws {
    ...
    for _ in 0 ..< 3 {
      let warmupPaths = try testHarness.pathEnumerationService
        .enumeratePaths(from: "A",
                        to: "C",
                        in: testHarness.fixtureGraph)
      _ = try await testHarness.scorePaths(warmupPaths,
                                           departureTime: fixedDeparture)
    }
    ...
    for i in 0 ..< 5 {
      let paths = try testHarness.pathEnumerationService.enumeratePaths(from: "A",
                                                                        to: "C",
                                                                        in: testHarness.fixtureGraph)
      let scores = try await testHarness.scorePaths(paths,
                                                    departureTime: fixedDeparture)
      ...
      #expect(memoryIncrease < tolerance,
              "Memory increase exceeded 240MB (with 20% tolerance)")
      print(
        "Iteration \(i): Memory usage: \(currentMemory / 1024 / 1024)MB"
      )
    }
    ...
    if let finalMemory = memoryReadings.last {
      ...
      #expect(totalIncrease < tolerance,
              "Final memory increase exceeded 60MB (with 20% tolerance)")
    } else {
  ...
  func endToEndPipelinePerformance() async throws {
    ...
    for (start, end, maxPaths) in testScenarios {
      do {
        ...
        let scores = try await testHarness.scorePaths(clamped,
                                                      departureTime: fixedDeparture)
        let duration = CFAbsoluteTimeGetCurrent() - startTime
        #expect(scores.count == clamped.count)

        performanceMetrics.record("EndToEnd(\(start)->\(end))",
                                  duration: duration)
      } catch {
...
private struct MultiPathTestHarness {
  ...
  func scorePaths(_ paths: [RoutePath], departureTime: Date) async throws
    -> [Double]
  {
...
private final class PerformanceMetrics {
  ...
  func summary() -> String {
    ...
    for (operation, times) in measurements {
      ...
      let max = times.max() ?? 0
      result +=
        "\(operation): avg=\(avg * 1000)ms, min=\(min * 1000)ms, max=\(max * 1000)ms\n"
    }
    return result
  }
}
...
private func fixedTopOfHour(reference: Date = Date()) -> Date {
  ...
  var comps = cal.dateComponents([.year, .month, .day, .hour],
                                 from: reference)
  comps.minute = 0
  comps.second = 0
  comps.nanosecond = 0
