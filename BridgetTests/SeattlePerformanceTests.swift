...
struct SeattlePerformanceTests {
    ...
    func algorithmParityDFSvsYen() throws {
        ...
        let dfsPaths = try dfsEnum.enumeratePaths(
            from: "A",
            to: "C",
            in: testHarness.fixtureGraph
        )
        let yensPaths = try yensEnum.enumeratePaths(
            from: "A",
            to: "C",
            in: testHarness.fixtureGraph
        )
    ...
    func cachePerformance_featureCacheOnly() async throws {
        ...
        let paths1 = try enumService.enumeratePaths(
            from: "A",
            to: "C",
            in: testHarness.fixtureGraph
        )
        let scores1 = try await scoringService.scorePaths(
            paths1,
        ...
        let paths2 = try enumService.enumeratePaths(
            from: "A",
            to: "C",
            in: testHarness.fixtureGraph
        )
        let scores2 = try await scoringService.scorePaths(
            paths2,
            departureTime: fixedDeparture
        )
        ...
        #expect(
            missesAfter2 >= missesAfter1,
            "Misses are cumulative counters; should not decrease"
        )
        #expect(
            hitRateAfter2 >= hitRateAfter1,
    ...
    func cachePerformance_pathMemoizationOnly() throws {
        ...
        _ = try? enumService.enumeratePaths(
            from: "A",
            to: "B",
            in: testHarness.fixtureGraph
        )
    ...
    func memoryStability() async throws {
        ...
        for _ in 0..<3 {
            ...
            _ = try await testHarness.scorePaths(
                warmupPaths,
                departureTime: fixedDeparture
            )
        }
        ...
        for i in 0..<5 {
            ...
            let scores = try await testHarness.scorePaths(
                paths,
                departureTime: fixedDeparture
            )
