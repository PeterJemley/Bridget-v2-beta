...
final class DataValidationTests {
    ...
    func goldenSampleFeatureVectorsValidation() async throws {
        ...
        for hour in 0..<24 {
            for bridgeId in [1, 2] {
                let timestamp = fixedCalendar.date(
                    byAdding: .hour,
                    value: hour,
                    to: baseTime
                )!
                ...
                for horizon in defaultHorizons {
                    ...
                    let weekday = fixedCalendar.component(
                        .weekday,
                        from: date
                    )
        ...
        let result = testValidationService.validate(
            features: testGoldenSampleFeatures
        )
    ...
    func emptyArraysValidation() async throws {
        ...
        #expect(
            emptyFeaturesResult.errors.contains("No feature vectors provided")
        )
    }
    ...
    func dSTBoundaryTimestamps() async throws {
        ...
        for minute in 0..<120 {
            let springTimestamp = fixedCalendar.date(
                byAdding: .minute,
                value: minute,
                to: springForwardBase
            )!
            let fallTimestamp = fixedCalendar.date(
                byAdding: .minute,
                value: minute,
                to: fallBackBase
            )!
    ...
    func naNValuesDetection() async throws {
        ...
        #expect(
            result.errors.contains {
                $0.contains("Found 1 NaN values in cross_k")
            }
        )
    }
    ...
    func infiniteValuesDetection() async throws {
        ...
        #expect(
            result.errors.contains {
                $0.contains("Found 1 infinite values in cross_n")
            }
        )
    }
    ...
    func nonMonotonicTimestamps() async throws {
        ...
        #expect(
            result.warnings.contains { $0.contains("Non-monotonic timestamp") }
        )
    }
    ...
    func featureVectorCyclicalRangeValidation() async throws {
        ...
        #expect(
            result.warnings.contains {
                $0.contains("Cyclical feature out of range")
            }
        )
    }
    ...
    func featureVectorInvalidTargets() async throws {
        ...
        #expect(result.isValid == false)
        #expect(
            result.errors.contains { $0.contains("Invalid target value: 2") }
        )
    }
    ...
    func featureVectorHorizonCoverage() async throws {
        ...
        let result = validationService.validate(
            features: limitedHorizonFeatures
        )

        #expect(result.isValid == true)  // Warnings don't make it invalid
        #expect(
            result.warnings.contains {
                $0.contains("Missing features for horizons: 9min")
            }
        )
    ...
    func dataQualityMetricsAggregation() async throws {
        ...
        #expect(
            result.dataQualityMetrics.nullCounts.isEmpty
                || result.dataQualityMetrics.nullCounts.values.allSatisfy {
                    $0 >= 0
                }
        )
        #expect(
            result.dataQualityMetrics.nanCounts.isEmpty
                || result.dataQualityMetrics.nanCounts.values.allSatisfy {
                    $0 >= 0
                }
        )
        #expect(
            result.dataQualityMetrics.infiniteCounts.isEmpty
                || result.dataQualityMetrics.infiniteCounts.values.allSatisfy {
                    $0 >= 0
                }
        )
        #expect(
            result.dataQualityMetrics.outlierCounts.isEmpty
                || result.dataQualityMetrics.outlierCounts.values.allSatisfy {
                    $0 >= 0
                }
        )
        #expect(
            result.dataQualityMetrics.rangeViolations.isEmpty
                || result.dataQualityMetrics.rangeViolations.values.allSatisfy {
                    $0 >= 0
                }
        )
    }
    ...
    func validationRateCalculation() async throws {
        ...
        let expectedRate =
            Double(result.validRecordCount) / Double(result.totalRecords)
    ...
    func shufflingPreservesValidationProperties() async throws {
        ...
        #expect(
            abs(originalResult.validationRate - shuffledResult.validationRate)
                < 0.001
        )
        ...
        #expect(
            abs(
                originalResult.dataQualityMetrics.dataCompleteness
                    - shuffledResult.dataQualityMetrics.dataCompleteness
            ) < 0.001
        )
        #expect(
            abs(
                originalResult.dataQualityMetrics.bridgeIDValidity
                    - shuffledResult.dataQualityMetrics.bridgeIDValidity
            ) < 0.001
        )
    ...
    func featureVectorHorizonInvariance() async throws {
        ...
        let originalResult = validationService.validate(
            features: goldenSampleFeatures
        )

        // Reorder features by horizon
        let reorderedFeatures = goldenSampleFeatures.sorted {
            $0.horizon_min < $1.horizon_min
        }
        let reorderedResult = validationService.validate(
    ...
    func duplicateRecordDetection() async throws {
        ...
        #expect(
            result.warnings.contains {
                $0.contains("duplicate") || $0.contains("Duplicate")
            }
        )
    }
    ...
    func unusualTimestampHandling() async throws {
        let unusualTicks = [
            ...
            ProbeTickRaw(
                v: 1,
                ts_utc: "2099-12-31T23:59:59Z",
                bridge_id: 3,
                cross_k: 0.4,
                cross_n: 0.9,
                via_routable: 0.7,
                via_penalty_sec: 35.0,
                gate_anom: 0.15,
                alternates_total: 1.0,
                alternates_avoid: 0.7,
                open_label: 0,
                detour_delta: 150.0,
                detour_frac: 0.4
            ),
        ...
        #expect(
            !result.errors.contains { $0.contains("Invalid timestamp format") }
        )
    }
    ...
    func leapSecondAndTimezoneHandling() async throws {
        let timezoneTicks = [
            ...
            ProbeTickRaw(
                v: 1,
                ts_utc: "2025-01-01T12:00:00-08:00",
                bridge_id: 2,
                cross_k: 0.6,
                cross_n: 1.1,
                via_routable: 0.9,
            ...
            ProbeTickRaw(
                v: 1,
                ts_utc: "2025-01-01T15:00:00-05:00",
                bridge_id: 4,
                cross_k: 0.3,
                cross_n: 0.8,
                via_routable: 0.6,
                via_penalty_sec: 40.0,
                gate_anom: 0.2,
                alternates_total: 1.5,
                alternates_avoid: 0.6,
                open_label: 1,
                detour_delta: 180.0,
                detour_frac: 0.5
            ),
    ...
    func allMissingFeaturesDetection() async throws {
        let allMissingTicks = [
            ProbeTickRaw(
                ...
                via_penalty_sec: nil,
                gate_anom: nil,
                alternates_total: nil,
                alternates_avoid: nil,
                open_label: 0,
                detour_delta: nil,
                detour_frac: nil
            ),
            ProbeTickRaw(
                v: 1,
                ts_utc: "2025-01-01T12:01:00Z",
                bridge_id: 2,
                cross_k: Double.nan,
                cross_n: Double.nan,
                via_routable: Double.nan,
                via_penalty_sec: Double.nan,
                gate_anom: Double.nan,
                alternates_total: Double.nan,
                alternates_avoid: Double.nan,
                open_label: 1,
                detour_delta: Double.nan,
        ...
        #expect(
            result.warnings.contains {
                $0.contains("High null rate") || $0.contains("missing ratio")
            }
        )
    }
    ...
    func allValuesNaNInfiniteForFields() async throws {
        ...
        let fieldSpecificTicks = [
            ...
            ProbeTickRaw(
                v: 1,
                ts_utc: "2025-01-01T12:03:00Z",
                bridge_id: 4,
                cross_k: 0.5,
                cross_n: 1.0,
                via_routable: 0.8,
                via_penalty_sec: Double.infinity,
                gate_anom: 0.1,
                alternates_total: 2.0,
                alternates_avoid: 0.5,
                open_label: 0,
        ...
        let result = isolatedValidationService.validate(
            ticks: fieldSpecificTicks
        )
        ...
        #expect(
            result.dataQualityMetrics.infiniteCounts["via_penalty_sec"] == 3
        )
        ...
        #expect(
            result.errors.contains {
                $0.contains("infinite") || $0.contains("Infinite")
            }
        )
        ...
        for bridgeId in 1...1000 {
            highCardinalityTicks.append(
                ProbeTickRaw(
                    v: 1,
                    ts_utc:
                        "2025-01-01T\(String(format: "%02d", bridgeId % 24)):00:00Z",
                    bridge_id: bridgeId,
                    cross_k: 0.5,
                    cross_n: 1.0,
                    via_routable: 0.8,
    ...
    func precisionLossHandling() async throws {
        let precisionTicks = [
            ...
            ProbeTickRaw(
                ...
                cross_k: 0.0000001,
                cross_n: 0.9999999,
                via_routable: 0.0000001,
                via_penalty_sec: 0.1,
                gate_anom: 0.0000001,
                alternates_total: 0.1,
                alternates_avoid: 0.9999999,
                open_label: 0,
                detour_delta: 0.1,
                detour_frac: 0.9999999
            ),
            ...
            ProbeTickRaw(
                ...
                ts_utc: "2025-01-01T12:01:00Z",
                bridge_id: 2,
                cross_k: 0.123456789123456789,
                cross_n: 1.987654321987654321,
                via_routable: 0.555555555555555555,
                via_penalty_sec: 29.999999999999999,
                gate_anom: 0.111111111111111111,
                alternates_total: 2.888888888888888888,
                alternates_avoid: 0.444444444444444444,
                open_label: 1,
                detour_delta: 119.999999999999999,
    ...
    func nonDefaultHorizonsHandling() async throws {
        let nonStandardFeatures = [
            ...
            FeatureVector(
                ...
                normal_speed: 55.0,
                target: 1
            ),  // Non-standard 11-minute horizon
        ]
        ...
        #expect(
            result.warnings.contains {
                $0.contains("Missing features for horizons")
                    || $0.contains("horizon")
            }
        )
        ...
        for iteration in 1...10 {
            ...
            for i in 1...50 {
                let randomTick = ProbeTickRaw(
                    v: 1 + (i % 2),  // Deterministic version
                    ts_utc: generateRandomTimestamp(),
                    bridge_id: 1 + (i % 20),  // Deterministic bridge ID
                    cross_k: deterministicOptionalDouble(
                        validRange: 0...2,
                        nilChance: 0.1,
                        nanChance: 0.05,
                        seed: i
                    ),
                    cross_n: deterministicOptionalDouble(
                        validRange: 0...3,
                        nilChance: 0.1,
                        nanChance: 0.05,
                        seed: i
                    ),
                    via_routable: deterministicOptionalDouble(
                        validRange: 0...1,
                        nilChance: 0.1,
                        nanChance: 0.05,
                        seed: i
                    ),
                    via_penalty_sec: deterministicOptionalDouble(
                        validRange: 0...100,
                        nilChance: 0.1,
                        nanChance: 0.05,
                        seed: i
                    ),
                    gate_anom: deterministicOptionalDouble(
                        validRange: 0...1,
                        nilChance: 0.1,
                        nanChance: 0.05,
                        seed: i
                    ),
                    alternates_total: deterministicOptionalDouble(
                        validRange: 0...10,
                        nilChance: 0.1,
                        nanChance: 0.05,
                        seed: i
                    ),
                    alternates_avoid: deterministicOptionalDouble(
                        validRange: 0...1,
                        nilChance: 0.1,
                        nanChance: 0.05,
                        seed: i
                    ...
                    detour_delta: deterministicOptionalDouble(
                        validRange: 0...300,
                        nilChance: 0.1,
                        nanChance: 0.05,
                        seed: i
                    ),
                    detour_frac: deterministicOptionalDouble(
                        validRange: 0...1,
                        nilChance: 0.1,
                        nanChance: 0.05
                    )
    ...
    func partialFailureSimulation() async throws {
        ...
        #expect(
            !allMessages.isEmpty,
            "Should have validation messages for invalid data"
        )

        // Should detect the invalid bridge ID (999) - check if it's flagged in any way
        #expect(
            result.bridgeCount < 4
                || allMessages.contains { $0.contains("999") },
        ...
        #expect(
            result.validRecordCount < 4
                || allMessages.contains { $0.contains("invalid") },
    ...
    func unknownFieldHandling() async throws {
        ...
        #expect(
            !result.warnings.contains {
                $0.contains("unknown") || $0.contains("Unknown")
            }
        )
    ...
    func dataDriftDetection() async throws {
        ...
        let driftedTicks = [
            ProbeTickRaw(
                v: 1,
                ts_utc: "2025-01-01T13:00:00Z",
                bridge_id: 1,
                cross_k: 1.5,
                cross_n: 2.0,
                via_routable: 0.3,  // Significantly different values
                via_penalty_sec: 80.0,
                gate_anom: 0.8,
                alternates_total: 8.0,
                alternates_avoid: 0.9,
                open_label: 0,
                detour_delta: 300.0,
                detour_frac: 0.8
            ),
            ProbeTickRaw(
                v: 1,
                ts_utc: "2025-01-01T13:01:00Z",
                bridge_id: 1,
                cross_k: 1.6,
                cross_n: 2.1,
                via_routable: 0.2,
                via_penalty_sec: 85.0,
                gate_anom: 0.9,
                alternates_total: 9.0,
                alternates_avoid: 0.95,
                open_label: 1,
                detour_delta: 320.0,
                detour_frac: 0.9
            ),
    ...
    func asyncValidationPerformance() async throws {
        ...
        for i in 1...500 {
            largeTicks.append(
                ProbeTickRaw(
                    v: 1,
                    ts_utc:
                        "2025-01-01T\(String(format: "%02d", i % 24)):00:00Z",
                    bridge_id: (i % 10) + 1,
                    cross_k: 0.5,
                    cross_n: 1.0,
                    via_routable: 0.8,
        ...
        let asyncResult = await validationService.validateAsync(
            ticks: largeTicks
        )
    ...
    func customValidatorIntegration() async throws {
        ...
        struct MockValidator: CustomValidator {
            ...
            func validate(features _: [FeatureVector]) async
        ...
        let testTicks = [
            ProbeTickRaw(
                v: 1,
                ts_utc: "2025-01-01T12:00:00Z",
                bridge_id: 150,  // Should trigger mock validator
                cross_k: 0.5,
                cross_n: 1.0,
                via_routable: 0.8,
                via_penalty_sec: 30.0,
    ...
    func runtimePluginManagement() async throws {
            ...
            func validate(ticks: [ProbeTickRaw]) async -> DataValidationResult {
                ...
                if ticks.count > 10 {
                    result.warnings.append(
                        "HighPriority: Large dataset detected (\(ticks.count) records)"
                    )
                }
                return result
            }
        ...
        struct MediumPriorityValidator: CustomValidator {
            ...
            func explain() -> String {
                return
                    "Medium priority validator that checks for high speed values"
            }
        }
        ...
        struct LowPriorityValidator: CustomValidator {
            ...
            func validate(ticks: [ProbeTickRaw]) async -> DataValidationResult {
                ...
                if !weekendRecords.isEmpty {
                    result.warnings.append(
                        "LowPriority: Found \(weekendRecords.count) weekend records"
                    )
                }
                return result
            }
            ...
            func explain() -> String {
                return
                    "Low priority validator that identifies weekend data patterns"
            }
        }
        ...
        let testTicks = [
            ProbeTickRaw(
                ...
                cross_k: 2.5,
                cross_n: 3.5,
                via_routable: 0.8,  // High speed values
                via_penalty_sec: 30.0,
                gate_anom: 0.1,
                alternates_total: 2.0,
                alternates_avoid: 0.5,
        ...
        for i in 3...15 {
            largeDataset.append(
                ProbeTickRaw(
                    v: 1,
                    ts_utc: "2025-01-01T12:\(String(format: "%02d", i)):00Z",
                    bridge_id: i,
                    cross_k: 0.5,
                    cross_n: 1.0,
                    via_routable: 0.8,
        ...
        #expect(
            result.warnings.contains {
                $0.contains("HighPriority: Large dataset")
            }
        )
        #expect(
            result.warnings.contains { $0.contains("MediumPriority: Found") }
        )

        // Test 3: Disable specific validator
        validationService.removeValidator(named: "MediumPriorityValidator")

        let resultAfterRemoval = await validationService.validateAsync(
            ticks: largeDataset
        )

        // Should still have high priority warning but not medium priority
        #expect(
            resultAfterRemoval.warnings.contains {
                $0.contains("HighPriority: Large dataset")
            }
        )
        #expect(
            !resultAfterRemoval.warnings.contains {
                $0.contains("MediumPriority: Found")
            }
        )
    ...
    func realSampleDataActionableFeedback() async throws {
        ...
        if !result.errors.isEmpty {
            for error in result.errors {
                #expect(
                    error.contains("Invalid") || error.contains("Found")
        ...
        if !result.warnings.isEmpty {
            for warning in result.warnings {
                #expect(
                    warning.contains("High") || warning.contains("Missing")
                        || warning.contains("Unbalanced")
    ...
    private func deterministicOptionalDouble(
    ...
    private func loadRealSampleData() throws -> [ProbeTickRaw] {
        ...
        if let url = bundle.url(
            forResource: "minutes_2025-08-12",
        ...
        if let url = bundle.url(
            forResource: "Samples/ndjson/minutes_2025-08-12",
            withExtension: "ndjson"
        ) {
            return try loadSampleDataFromURL(url)
        }
    ...
    func edgeCaseCrashPrevention() async throws {
        ...
        let duplicateResult = validationService.validate(
            ticks: duplicateTimeTicks
        )
        ...
        #expect(
            invalidResult.errors.contains { $0.contains("Invalid timestamp") }
        )
        ...
        #expect(
            mixedResult.errors.contains { $0.contains("Invalid timestamp") }
        )
    ...
    func leapSecondHandling() async throws {
        ...
        let leapSecondTicks = [
            ProbeTickRaw(
                v: 1,
                ts_utc: "2025-06-30T23:59:60Z",
                bridge_id: 1,  // Leap second
                cross_k: 0.5,
                cross_n: 1.0,
                via_routable: 0.8,
                via_penalty_sec: 30.0,
            ...
            ProbeTickRaw(
                v: 1,
                ts_utc: "2025-06-30T23:59:59Z",
                bridge_id: 1,  // Normal second
                cross_k: 0.5,
                cross_n: 1.0,
                via_routable: 0.8,
                via_penalty_sec: 30.0,
            ...
            ProbeTickRaw(
                v: 1,
                ts_utc: "2025-07-01T00:00:00Z",
                bridge_id: 1,  // Next day
                cross_k: 0.5,
                cross_n: 1.0,
                via_routable: 0.8,
                via_penalty_sec: 30.0,
    ...
    func parsingFailureHandling() async throws {
        ...
        let malformedTicks = [
            ...
            ProbeTickRaw(
                v: 1,
                ts_utc: "2025-01-01T12:00:00",
                bridge_id: 1,  // Missing timezone
                cross_k: 0.5,
                cross_n: 1.0,
                via_routable: 0.8,
                via_penalty_sec: 30.0,
            ...
            ProbeTickRaw(
                v: 1,
                ts_utc: "2025-13-01T12:00:00Z",
                bridge_id: 1,  // Invalid month
                cross_k: 0.5,
                cross_n: 1.0,
                via_routable: 0.8,
                via_penalty_sec: 30.0,
