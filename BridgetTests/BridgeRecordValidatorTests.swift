...
struct BridgeRecordValidatorTests {
  ...
  func rejectsOutOfRangeOpenDate() async throws {
    ...
    #expect(
      result == .outOfRangeOpenDate(expectedDate), "Out of range open date should be rejected")
  }
  ...
  func rejectsCloseDateNotAfterOpenDate() async throws {
    ...
    let closeDate =
      ...
      ?? Date()
    #expect(
      result == .closeDateNotAfterOpenDate(open: openDate, close: closeDate),
  ...
  func rejectsGeospatialMismatch() async throws {
    ...
    #expect(
      result
        == .geospatialMismatch(
          expectedLat: 47.542213439941406, expectedLon: -122.33446502685547, actualLat: 47.0,
