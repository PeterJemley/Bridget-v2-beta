...
struct SeattleDrawbridgesTests {
    ...
    func bridgeCount() {
        #expect(
            SeattleDrawbridges.count == 7,
            "Should have exactly 7 Seattle drawbridges"
        )
        #expect(
            SeattleDrawbridges.allBridges.count == 7,
            "All bridges array should have 7 bridges"
        )
        #expect(
            SeattleDrawbridges.BridgeID.allCases.count == 7,
    ...
    func invalidBridgeID() {
        ...
        #expect(
            !SeattleDrawbridges.isValidBridgeID("999"),
            "Should reject invalid bridge ID"
        )
        #expect(
            SeattleDrawbridges.isValidBridgeID("2"),
    ...
    func bridgeLocationsCompatibility() {
        #expect(
            SeattleDrawbridges.bridgeLocations.count == 7,
        ...
        #expect(
            abs(location.lat - 47.6598) < 0.0001,
            "Ballard latitude should match"
        )
        #expect(
            abs(location.lon - -122.3762) < 0.0001,
    ...
    func bridgeIDExtensions() {
        ...
        #expect(
            ballardID.displayName == "Ballard Bridge",
            "Display name should work"
        )

        let coordinate = ballardID.coordinate
        #expect(
            abs(coordinate.latitude - 47.6598) < 0.0001,
            "Coordinate should work (latitude)"
        )
        #expect(
            abs(coordinate.longitude - -122.3762) < 0.0001,
    ...
    func nonCanonicalBridgeIDRejection() {
        ...
        for nonCanonicalID in nonCanonicalIDs {
            ...
            #expect(
                SeattleDrawbridges.bridgeInfo(for: nonCanonicalID) == nil,
    ...
    func canonicalBridgeIDAcceptance() {
        for bridgeID in SeattleDrawbridges.BridgeID.allIDs {
            ...
            #expect(
                SeattleDrawbridges.bridgeInfo(for: bridgeID) != nil,
    ...
    func bridgeIDValidationEdgeCases() {
        #expect(
            !SeattleDrawbridges.isValidBridgeID(""),
        ...
        #expect(
            !SeattleDrawbridges.isValidBridgeID("1.0"),
            "Decimal should be rejected"
        )
        #expect(
            !SeattleDrawbridges.isValidBridgeID("1a"),
    ...
    func canonicalBridgeIDValidation() {
        ...
        #expect(
            SeattleDrawbridges.isCanonicalBridgeID("1"),
            "First Avenue South Bridge should be canonical"
        )
        #expect(
            SeattleDrawbridges.isCanonicalBridgeID("2"),
            "Ballard Bridge should be canonical"
        )
        #expect(
            SeattleDrawbridges.isCanonicalBridgeID("29"),
            "South Park Bridge should be canonical"
        )

        // Non-canonical bridge IDs
        #expect(
            !SeattleDrawbridges.isCanonicalBridgeID("0"),
        ...
        #expect(
            !SeattleDrawbridges.isCanonicalBridgeID("bridge1"),
            "Synthetic test ID should not be canonical"
        )
        #expect(
            !SeattleDrawbridges.isCanonicalBridgeID(""),
    ...
    func syntheticTestBridgeIDValidation() {
        ...
        #expect(
            SeattleDrawbridges.isSyntheticTestBridgeID("bridge1"),
            "bridge1 should be synthetic test ID"
        )
        #expect(
            SeattleDrawbridges.isSyntheticTestBridgeID("bridge2"),
        ...
        #expect(
            !SeattleDrawbridges.isSyntheticTestBridgeID("1"),
        ...
        #expect(
            !SeattleDrawbridges.isSyntheticTestBridgeID("bridge1a"),
            "bridge1a should not be synthetic"
        )
        #expect(
            !SeattleDrawbridges.isSyntheticTestBridgeID(""),
    ...
    func acceptedBridgeIDValidation() {
        ...
        #expect(
            SeattleDrawbridges.isAcceptedBridgeID("1"),
            "Canonical bridge ID should be accepted"
        )
        #expect(
            SeattleDrawbridges.isAcceptedBridgeID("29"),
            "Canonical bridge ID should be accepted"
        )
        #expect(
        ...
        #expect(
            !SeattleDrawbridges.isAcceptedBridgeID("999"),
