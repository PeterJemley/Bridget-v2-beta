...
final class SeattleDrawbridgesTests: XCTestCase {
  func testBridgeCount() {
    XCTAssertEqual(SeattleDrawbridges.count,
                   7,
                   "Should have exactly 7 Seattle drawbridges")
    XCTAssertEqual(SeattleDrawbridges.allBridges.count,
                   7,
                   "All bridges array should have 7 bridges")
    XCTAssertEqual(SeattleDrawbridges.BridgeID.allCases.count,
                   7,
  ...
  func testBridgeIDs() {
    ...
    XCTAssertTrue(actualIDs.contains("1"),
                  "Should contain First Avenue South Bridge ID")
    XCTAssertTrue(actualIDs.contains("2"),
                  "Should contain Ballard Bridge ID")
    XCTAssertTrue(actualIDs.contains("3"),
                  "Should contain Fremont Bridge ID")
    XCTAssertTrue(actualIDs.contains("4"),
                  "Should contain Montlake Bridge ID")
    XCTAssertTrue(actualIDs.contains("6"),
                  "Should contain Lower Spokane Bridge ID")
    XCTAssertTrue(actualIDs.contains("21"),
                  "Should contain University Bridge ID")
    XCTAssertTrue(actualIDs.contains("29"),
                  "Should contain South Park Bridge ID")
  }
  ...
  func testInvalidBridgeID() {
    ...
    XCTAssertFalse(SeattleDrawbridges.isValidBridgeID("999"),
                   "Should reject invalid bridge ID")
    XCTAssertTrue(SeattleDrawbridges.isValidBridgeID("2"),
  ...
  func testBridgeLocationsCompatibility() {
    ...
    XCTAssertEqual(SeattleDrawbridges.bridgeLocations.count,
                   7,
    ...
    XCTAssertEqual(location.lat,
                   47.6598,
                   accuracy: 0.0001,
                   "Ballard latitude should match")
    XCTAssertEqual(location.lon,
                   -122.3762,
                   accuracy: 0.0001,
  ...
  func testBridgeNamesCompatibility() {
    ...
    XCTAssertEqual(SeattleDrawbridges.bridgeNames.count,
                   7,
                   "Should have 7 bridge names")

    let ballardName = SeattleDrawbridges.bridgeNames["2"]
    XCTAssertEqual(ballardName,
                   "Ballard Bridge",
  ...
  func testBridgeIDExtensions() {
    ...
    XCTAssertEqual(ballardID.displayName,
                   "Ballard Bridge",
                   "Display name should work")

    let coordinate = ballardID.coordinate
    XCTAssertEqual(coordinate.latitude,
                   47.6598,
                   accuracy: 0.0001,
                   "Coordinate should work")
  ...
  func testNonCanonicalBridgeIDRejection() {
    ...
    for nonCanonicalID in nonCanonicalIDs {
      XCTAssertFalse(SeattleDrawbridges.isValidBridgeID(nonCanonicalID),
                     "Non-canonical ID '\(nonCanonicalID)' should be rejected")
      XCTAssertNil(SeattleDrawbridges.bridgeInfo(for: nonCanonicalID),
  ...
  func testCanonicalBridgeIDAcceptance() {
    ...
    for bridgeID in SeattleDrawbridges.BridgeID.allIDs {
      XCTAssertTrue(SeattleDrawbridges.isValidBridgeID(bridgeID),
                    "Canonical ID '\(bridgeID)' should be accepted")
      XCTAssertNotNil(SeattleDrawbridges.bridgeInfo(for: bridgeID),
  ...
  func testBridgeIDValidationEdgeCases() {
    // Test edge cases
    XCTAssertFalse(SeattleDrawbridges.isValidBridgeID(""),
                   "Empty string should be rejected")
    XCTAssertFalse(SeattleDrawbridges.isValidBridgeID(" "),
                   "Whitespace should be rejected")
    XCTAssertFalse(SeattleDrawbridges.isValidBridgeID("1.0"),
                   "Decimal should be rejected")
    XCTAssertFalse(SeattleDrawbridges.isValidBridgeID("1a"),
  ...
  func testCanonicalBridgeIDValidation() {
    ...
    XCTAssertTrue(SeattleDrawbridges.isCanonicalBridgeID("1"),
                  "First Avenue South Bridge should be canonical")
    XCTAssertTrue(SeattleDrawbridges.isCanonicalBridgeID("2"),
                  "Ballard Bridge should be canonical")
    XCTAssertTrue(SeattleDrawbridges.isCanonicalBridgeID("29"),
                  "South Park Bridge should be canonical")

    // Test non-canonical bridge IDs
    XCTAssertFalse(SeattleDrawbridges.isCanonicalBridgeID("0"),
                   "Non-existent bridge ID should not be canonical")
    XCTAssertFalse(SeattleDrawbridges.isCanonicalBridgeID("999"),
                   "Non-existent bridge ID should not be canonical")
    XCTAssertFalse(SeattleDrawbridges.isCanonicalBridgeID("bridge1"),
                   "Synthetic test ID should not be canonical")
    XCTAssertFalse(SeattleDrawbridges.isCanonicalBridgeID(""),
                   "Empty string should not be canonical")
  }

  func testSyntheticTestBridgeIDValidation() {
    // Test synthetic test bridge IDs
    XCTAssertTrue(SeattleDrawbridges.isSyntheticTestBridgeID("bridge1"),
                  "bridge1 should be synthetic test ID")
    XCTAssertTrue(SeattleDrawbridges.isSyntheticTestBridgeID("bridge2"),
                  "bridge2 should be synthetic test ID")
    XCTAssertTrue(SeattleDrawbridges.isSyntheticTestBridgeID("bridge999"),
                  "bridge999 should be synthetic test ID")

    // Test non-synthetic bridge IDs
    XCTAssertFalse(SeattleDrawbridges.isSyntheticTestBridgeID("1"),
                   "Canonical bridge ID should not be synthetic")
    XCTAssertFalse(SeattleDrawbridges.isSyntheticTestBridgeID("bridge"),
                   "bridge without number should not be synthetic")
    XCTAssertFalse(SeattleDrawbridges.isSyntheticTestBridgeID("bridge1a"),
                   "bridge1a should not be synthetic")
    XCTAssertFalse(SeattleDrawbridges.isSyntheticTestBridgeID(""),
                   "Empty string should not be synthetic")
  }

  func testAcceptedBridgeIDValidation() {
    // Test with allowSynthetic: false (default)
    XCTAssertTrue(SeattleDrawbridges.isAcceptedBridgeID("1"),
                  "Canonical bridge ID should be accepted")
    XCTAssertTrue(SeattleDrawbridges.isAcceptedBridgeID("29"),
                  "Canonical bridge ID should be accepted")
    XCTAssertFalse(SeattleDrawbridges.isAcceptedBridgeID("bridge1"),
                   "Synthetic test ID should not be accepted by default")
    XCTAssertFalse(SeattleDrawbridges.isAcceptedBridgeID("999"),
                   "Non-canonical ID should not be accepted")

    // Test with allowSynthetic: true
    XCTAssertTrue(SeattleDrawbridges.isAcceptedBridgeID("1", allowSynthetic: true),
                  "Canonical bridge ID should be accepted")
    XCTAssertTrue(SeattleDrawbridges.isAcceptedBridgeID("bridge1",
                                                        allowSynthetic: true),
                  "Synthetic test ID should be accepted when allowed")
    XCTAssertTrue(SeattleDrawbridges.isAcceptedBridgeID("bridge999",
                                                        allowSynthetic: true),
                  "Synthetic test ID should be accepted when allowed")
    XCTAssertFalse(SeattleDrawbridges.isAcceptedBridgeID("999", allowSynthetic: true),
  ...
  func testBridgeIDSetConsistency() {
    ...
    XCTAssertEqual(allIDs,
                   allCases,
                   "allIDs and allCases should be identical")
    XCTAssertEqual(allIDs,
                   bridgeInfoIDs,
    ...
    XCTAssertEqual(allIDs,
                   nameKeys,
                   "allIDs and nameKeys should be identical")
  }
}
