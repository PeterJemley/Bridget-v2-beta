...
struct PipelineParityValidatorTests {
  ...
  private func createBaselineMetrics() -> BaselineMetrics {
    ...
    let fieldRanges: [String: ValueRange] = [
      "cross_k": ValueRange(min: 0,
                            max: 100,
                            mean: 45.2,
                            description: "Baseline cross_k range"),
      "cross_n": ValueRange(min: 1,
                            max: 200,
                            mean: 98.7,
                            description: "Baseline cross_n range"),
      "via_penalty_sec": ValueRange(min: 0,
                                    max: 300,
                                    mean: 45.8,
                                    description: "Baseline penalty range"),
      "gate_anom": ValueRange(min: 0,
                              max: 1,
                              mean: 0.5,
                              description: "Baseline anomaly range"),
      "alternates_total": ValueRange(min: 1,
                                     max: 5,
                                     mean: 3.2,
    ...
    async throws
  {
    ...
    #expect(result.affectedModule == .bridgeDataProcessor,
    ...
    async throws
  {
    ...
    let scaledFieldRanges: [String: ValueRange] = [
      "cross_k": ValueRange(min: 0,
                            max: 120,
                            mean: 54.2,
                            description: "Scaled cross_k range"),  // +20%
      "cross_n": ValueRange(min: 1,
                            max: 200,
                            mean: 98.7,
    ...
    async throws
  {
    ...
    currentOutput = createCurrentOutput()
    let renamedSchema = DataSchema(fields: [
      "v", "ts_utc", "bridge_id", "cross_k", "cross_n",
      "via_routable", "via_penalty_sec",
      "gate_anom", "alternates_total", "alternates_avoid_span",
      "free_eta_sec", "via_eta_sec",
    ...
    async throws
  {
    ...
    let nanFieldRanges: [String: ValueRange] = [
      "cross_k": ValueRange(min: Double.nan,
                            max: 100,
                            mean: 45.2,
    ...
    #expect(nanChange?.severity == .critical,
  ...
  mutating func moduleImpactAnalysisIdentifiesCorrectModule() async throws {
    ...
    #expect(result.likelyModules.isEmpty == false,
            "Ranked module list should not be empty")
    #expect(result.likelyModules.first == .bridgeDataProcessor,
