...
struct PipelineParityValidatorTests {
    ...
    private func createBaselineMetrics() -> BaselineMetrics {
        let timeDistribution = TimeDistribution(
            hourlyDistribution: createHourlyDistribution(),
        ...
        let fieldRanges: [String: ValueRange] = [
            "cross_k": ValueRange(
                ...
                max: 100,
                mean: 45.2,
                description: "Baseline cross_k range"
            ),
            "cross_n": ValueRange(
                min: 1,
                max: 200,
                mean: 98.7,
                description: "Baseline cross_n range"
            ),
            "via_penalty_sec": ValueRange(
                min: 0,
                max: 300,
                mean: 45.8,
                description: "Baseline penalty range"
            ),
            "gate_anom": ValueRange(
                min: 0,
                max: 1,
                mean: 0.5,
                description: "Baseline anomaly range"
            ),
            "alternates_total": ValueRange(
                min: 1,
                max: 5,
                mean: 3.2,
                description: "Baseline alternates range"
            ),
        ]

        let fieldDistributions: [String: FieldDistribution] = [
            "bridge_id": FieldDistribution(
                values: ["1": 2880, "2": 2880, "3": 2880],
                description: "Baseline bridge distribution"
            ),
            "via_routable": FieldDistribution(
                values: ["0": 4320, "1": 4320],
                description: "Baseline routable distribution"
            ),
            "open_label": FieldDistribution(
                values: ["0": 6480, "1": 2160],
        ...
        let schema = DataSchema(
            fields: [
                "v", "ts_utc", "bridge_id", "cross_k", "cross_n",
                "via_routable", "via_penalty_sec",
                "gate_anom", "alternates_total", "alternates_avoid_span",
                "free_eta_sec", "via_eta_sec",
                "open_label",
            ],
            fieldTypes: [
                "v": "Int", "ts_utc": "String", "bridge_id": "Int",
                "cross_k": "Int16", "cross_n": "Int16",
                "via_routable": "Int", "via_penalty_sec": "Int32",
                "gate_anom": "Double",
                "alternates_total": "Int16",
                "alternates_avoid_span": "Int16", "free_eta_sec": "Int32?",
                "via_eta_sec": "Int32?",
                "open_label": "Int",
            ]
        )
    ...
    mutating func parityPassesWhenNoCodeChanged() async throws {
        ...
        #expect(
            result.isParity == true,
            "Parity should pass when outputs are identical"
        )
        #expect(
        ...
        #expect(
            result.detectedChanges.isEmpty == true,
    ...
    mutating func shapeDriftRemovesOneFeatureExpectsCriticalChange()
        async throws
    {
        try await setUp()
        ...
        #expect(
            ...
            "Parity should fail when field count changes"
        )
        #expect(result.failureReason != nil, "Failure reason should be present")
        #expect(
            result.affectedModule == .bridgeDataProcessor,
            "BridgeDataProcessor should be identified"
        )

        // Check for specific shape changes
        let shapeChanges = result.detectedChanges.filter {
            $0.changeType == .shape
        }
        #expect(
            shapeChanges.isEmpty == false,
            "Shape changes should be detected"
        )

        let fieldCountChange = shapeChanges.first {
            $0.affectedField == "field_count"
        }
        #expect(
            fieldCountChange != nil,
            "Field count change should be detected"
        )
        #expect(
            fieldCountChange?.severity == .critical,
            "Field count change should be critical"
        )
        ...
        async throws
    {
        ...
        let result = try await relaxedValidator.validateParity(
            baseline: baselineMetrics,
            current: currentOutput,
            sample: goldenSample
        )
        ...
        #expect(
            countChanges.isEmpty == false,
            "Count changes should be detected"
        )

        let bridge1CountChange = countChanges.first {
            $0.affectedField == "bridge_1_records"
        }
        #expect(
            bridge1CountChange != nil,
            "Bridge 1 count change should be detected"
        )
        ...
        #expect(
            bridge1CountChange?.metadata["current_count"]
                == "\(reducedBridge1Count)"
        )
    ...
    @Test(
        "Range drift scales one feature by 10% expects warning on value range"
    )
    mutating func
        rangeDriftScalesOneFeatureBy10PercentExpectsWarningOnValueRange()
        async throws
    {
        try await setUp()
        ...
        let scaledFieldRanges: [String: ValueRange] = [
            "cross_k": ValueRange(
                ...
                max: 120,
                mean: 54.2,
                description: "Scaled cross_k range"
            ),  // +20%
            "cross_n": ValueRange(
                min: 1,
                max: 200,
                mean: 98.7,
                description: "Current cross_n range"
            ),
            "via_penalty_sec": ValueRange(
                min: 0,
                max: 300,
                mean: 45.8,
                description: "Current penalty range"
            ),
            "gate_anom": ValueRange(
                min: 0,
                max: 1,
                mean: 0.5,
                description: "Current anomaly range"
            ),
            "alternates_total": ValueRange(
                min: 1,
                max: 5,
                mean: 3.2,
                description: "Current alternates range"
            ),
        ]
        ...
        #expect(
            result.isParity == false,
            "Parity should fail when value ranges differ"
        )

        // Check for specific range changes
        let rangeChanges = result.detectedChanges.filter {
            $0.changeType == .range
        }
        #expect(
            rangeChanges.isEmpty == false,
            "Range changes should be detected"
        )

        let crossKRangeChange = rangeChanges.first {
            $0.affectedField == "cross_k"
        }
        #expect(
            crossKRangeChange != nil,
            "cross_k range change should be detected"
        )
        #expect(
            crossKRangeChange?.severity == .minor,
            "Range change should be minor"
        )

        // Check metadata
        #expect(
            crossKRangeChange?.metadata["baseline_range"]
                == "min:0.0, max:100.0, mean:45.2"
        )
        #expect(
            crossKRangeChange?.metadata["current_range"]
                == "min:0.0, max:120.0, mean:54.2"
        )
    ...
    @Test(
        "Schema drift renames field expects critical with schema diff details"
    )
    mutating func schemaDriftRenamesFieldExpectsCriticalWithSchemaDiffDetails()
        async throws
    {
        try await setUp()
        ...
        #expect(
            result.isParity == false,
            "Parity should fail when schema differs"
        )

        // Check for specific schema changes
        let schemaChanges = result.detectedChanges.filter {
            $0.changeType == .schema
        }
        #expect(
            schemaChanges.isEmpty == false,
            "Schema changes should be detected"
        )

        let schemaHashChange = schemaChanges.first {
            $0.affectedField == "schema_hash"
        }
        #expect(
            schemaHashChange != nil,
            "Schema hash change should be detected"
        )
        #expect(
            schemaHashChange?.severity == .critical,
            "Schema hash change should be critical"
        )

        // Check metadata
        #expect(
            schemaHashChange?.metadata["baseline_hash"]
                == "baseline_schema_hash_12345"
        )
        #expect(
            schemaHashChange?.metadata["current_hash"]
                == "renamed_schema_hash_11111"
        )
        #expect(
            schemaHashChange?.metadata["hash_difference"]
                == "Schema structure has changed"
        )
    }
    ...
    @Test(
        "Performance drift injects 30% slowdown expects warning based on config"
    )
    mutating func
        performanceDriftInjects30PercentSlowdownExpectsWarningBasedOnConfig()
        async throws
    {
        try await setUp()
        ...
        #expect(
            result.isParity == false,
            "Parity should fail when performance differs beyond tolerance"
        )

        // Check for specific performance changes
        let performanceChanges = result.detectedChanges.filter {
            $0.changeType == .performance
        }
        #expect(
            performanceChanges.isEmpty == false,
            "Performance changes should be detected"
        )

        let pipelineTimeChange = performanceChanges.first {
            $0.affectedField == "pipeline_time"
        }
        #expect(
            pipelineTimeChange != nil,
            "Pipeline time change should be detected"
        )
        #expect(
            pipelineTimeChange?.severity == .minor,
            "Performance change should be minor"
        )

        // Check metadata
        #expect(pipelineTimeChange?.metadata["baseline_time"] == "2.1")
        #expect(
            pipelineTimeChange?.metadata["current_time"]
                == "\(slowerPipelineTime)"
        )
    ...
    mutating func relaxedConfigurationAllowsSmallerChanges() async throws {
        ...
        #expect(
            result.isParity == true,
            "Parity should pass with relaxed config for small changes"
        )
    }
    ...
    mutating func naNInfValuesTriggerAutomaticCriticalInRangeValidation()
        async throws
    {
        try await setUp()
        ...
        #expect(
            result.isParity == false,
            "Parity should fail when NaN values are detected"
        )
        ...
        #expect(
            nanChange?.severity == .critical,
            "NaN detection should be critical"
        )
    ...
    mutating func zeroBaselineAndCurrentDefineRelativeDeltaAsZero() async throws
    {
        ...
        let zeroFieldRanges: [String: ValueRange] = [
            "cross_k": ValueRange(
                ...
                max: 0,
                mean: 0,
                description: "Zero cross_k range"
            ),
            "cross_n": ValueRange(
                min: 0,
                max: 0,
                mean: 0,
                description: "Zero cross_n range"
            ),
            "via_penalty_sec": ValueRange(
                min: 0,
                max: 300,
        ...
        #expect(
            result.isParity == false,
            "Parity should fail due to range differences"
        )
    ...
    mutating func moduleImpactAnalysisIdentifiesCorrectModule() async throws {
        ...
        currentOutput = createCurrentOutput()
        currentOutput = CurrentOutput(
            outputStructure: "Modified NDJSON",
            fieldCount: 12,
            totalRecords: currentOutput.totalRecords,
            bridgeRecordCounts: currentOutput.bridgeRecordCounts,
            timeDistribution: currentOutput.timeDistribution,
        ...
        #expect(
            result.affectedModule == .bridgeDataProcessor,
            "BridgeDataProcessor should be identified for shape changes"
        )
        #expect(
            result.likelyModules.isEmpty == false,
            "Ranked module list should not be empty"
        )
        #expect(
            result.likelyModules.first == .bridgeDataProcessor,
            "BridgeDataProcessor should be first in ranked list"
        )
    }
    ...
    mutating func parityGateFacadeRunsValidation() async throws {
        ...
        let result = try await ParityGate.run(
        ...
        #expect(
            result.isParity == true,
            "Parity should pass for identical outputs"
        )
    }
}
