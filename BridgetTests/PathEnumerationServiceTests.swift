...
struct PathEnumerationServiceTests {
  ...
  func phase1SimpleFixture() throws {
    ...
    #expect(service.validatePaths(foundPaths),
    ...
    #expect(path1Nodes.contains(["A", "B", "C"]),
            "Should contain A->B->C path")
    #expect(path1Nodes.contains(["A", "D", "C"]),
  ...
  func phase1ComplexFixture() throws {
    ...
    #expect(pathNodes.contains(["A", "E", "D"]),
            "Should contain A->E->D path")
    #expect(pathNodes.contains(["A", "C", "D"]),
            "Should contain A->C->D path")
    #expect(pathNodes.contains(["A", "B", "D"]),
            "Should contain A->B->D path")
    #expect(pathNodes.contains(["A", "D"]),
  ...
  func shortestPath() throws {
    ...
    #expect(shortestPath?.nodes == ["A", "B", "D"],
            "Shortest path should be A->B->D (500s)")
    #expect(shortestPath?.totalTravelTime == 500,
    ...
    for startNode in graph.nodes {
      for endNode in graph.nodes {
        if startNode.id != endNode.id {
          ...
          for pathA in pathsA {
            #expect(pathsB.contains { $0.nodes == pathA.nodes },
  ...
  func phase2PruningEffectiveness() throws {
    ...
    #expect(restrictivePaths.count <= permissivePaths.count,
    ...
    for restrictivePath in restrictivePaths {
      #expect(permissivePaths.contains { $0.nodes == restrictivePath.nodes },
