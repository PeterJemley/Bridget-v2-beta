...
struct YensAlgorithmTests {
    ...
    func yensFindsKSortedPaths() throws {
        ...
        for i in 1..<paths.count {
            #expect(
                paths[i - 1].totalTravelTime <= paths[i].totalTravelTime,
    ...
    func yensWithDFSComparison() throws {
        ...
        let yensPaths = try yensService.enumeratePaths(
            from: "A",
            to: "E",
            in: testGraph
        )
    ...
    func yensWithConstraints() throws {
        let constrainedConfig = MultiPathConfig(
            pathEnumeration: PathEnumConfig(
                ...
                kShortestPaths: 10,
                randomSeed: 42,
                maxTimeOverShortest: 100
            )  // Only within 100s of shortest
        )
        ...
        for path in paths {
            ...
            if let shortestTime = paths.first?.totalTravelTime {
                ...
                #expect(
                    path.totalTravelTime <= maxAllowed,
