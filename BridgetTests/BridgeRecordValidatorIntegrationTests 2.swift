...
struct BridgeRecordValidatorIntegrationTests2 {
  ...
  private var transformService = DefaultCoordinateTransformService(enableLogging: true)

  private func makeValidator() -> BridgeRecordValidator {
    BridgeRecordValidator(knownBridgeIDs: knownBridgeIDs,
  ...
  func endToEndValidatorAcceptance_bridge1() throws {
    ...
    let testRecord = BridgeOpeningRecord(entitytype: "bridge",
    ...
    #expect(failure == nil, "Bridge record should pass validation after transformation")
  }
  ...
  func transformationImprovesProximityViaValidator() throws {
    ...
    for c in cases {
      ...
      let rec = BridgeOpeningRecord(entitytype: "bridge",
      ...
      #expect(failure == nil, "Record for bridge \(c.bridgeId) should pass after transformation")

      // Independently measure improvement using the transform service
      let originalDistance = haversineDistanceMeters(lat1: c.api.lat, lon1: c.api.lon,
                                                     lat2: c.expected.lat, lon2: c.expected.lon)

      let t = transformService.transformToReferenceSystem(latitude: c.api.lat,
                                                          longitude: c.api.lon,
                                                          from: .seattleAPI,
                                                          bridgeId: c.bridgeId)
      #expect(t.success, "Transform should succeed for bridge \(c.bridgeId)")

      let tLat = try #require(t.transformedLatitude)
      let tLon = try #require(t.transformedLongitude)
      let transformedDistance = haversineDistanceMeters(lat1: tLat, lon1: tLon,
                                                        lat2: c.expected.lat, lon2: c.expected.lon)

      #expect(transformedDistance < originalDistance,
              "Transformation should improve proximity for bridge \(c.bridgeId)")
      #expect(transformedDistance < c.maxDistanceAfterTransformMeters,
      ...
      print("   Improvement: \(Int(originalDistance - transformedDistance))m")
    }
  }
  ...
  func validatorPerformanceBudget() throws {
    ...
    #expect(avg < 0.01, "Average validation time should be < 10ms, got \(avg * 1000)ms")
    print("📍 Validator performance: \(String(format: "%.3f", avg * 1000))ms per validation")
  }
