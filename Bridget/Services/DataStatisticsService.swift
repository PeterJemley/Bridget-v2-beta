...
public class DataStatisticsService {
  ...
  private func generateSummary(from ticks: [ProbeTickRaw]) -> DataSummary {
    ...
    return DataSummary(totalRecords: total,
                       uniqueBridges: uniqueBridges,
                       dateRange: DateRange(firstTimestamp: first,
                                            lastTimestamp: last),
    ...
    -> [Int: BridgeStatistics]
  {
    ...
    for (bridgeID, items) in groups {
      ...
      result[bridgeID] = BridgeStatistics(bridgeID: bridgeID,
    ...
    -> HorizonStatistics
  {
    ...
    return HorizonStatistics(availableHorizons: available,
    ...
    -> TimeStatistics
  {
    ...
    return TimeStatistics(countsByMinute: [:],
                          countsByHour: [:],
                          countsByDayOfWeek: [:],
                          peakActivityTimes: [:],
                          lowActivityTimes: [:])
    ...
    -> HorizonStatistics
  {
    ...
    let featuresByBridge = Dictionary(grouping: features,
                                      by: { $0.bridge_id })
