...
class BridgeDataService {
    ...
    struct TransformationConfig: Codable, Equatable {
        var enabled: Bool
    ...
    struct TransformationMetrics: Codable, Equatable {
        var totalCoordinates: Int
        var transformedCoordinates: Int
        var skippedTransformations: Int
        var transformationFailures: Int
        var averageConfidence: Double
        var processingTimeSeconds: Double

        static let zero = TransformationMetrics(
            totalCoordinates: 0,
    ...
    func loadHistoricalData() async throws -> (
        [BridgeStatusModel], [ValidationFailure]
    ) {
        ...
        do {
                ...
                )
            {
                ...
                if transformationConfig.enableMetrics {
                    ...
                    transformationMetrics.processingTimeSeconds += max(
                        0.0,
                        elapsed
                    )
                }
    ...
    private func fetchFromNetwork() async throws -> Data {
        ...
        while hasMoreData {
            ...
            let batchData = try await fetchBatch(
                offset: offset,
                limit: batchSize
            )
