...
class BridgeDataService {
    ...
    private func loadFromCache<T: Codable>(_ type: T.Type, for key: String)
        -> T?
    {
    ...
    func loadHistoricalData() async throws -> [BridgeStatusModel] {
        ...
        if let cachedBridges: [BridgeStatusModel] = loadFromCache(
    ...
    private func processHistoricalData(_ data: Data) throws
        -> [BridgeStatusModel]
    {
        ...
        do {
            bridgeRecords = try decoder.decode(
        ...
        for record in bridgeRecords {
            ...
            guard let openDate = record.openDate else {
                #if DEBUG
                    print(
                        "Skipping record with invalid date: \(record.opendatetime)"
                    )
                #endif
                skippedCount += 1
                continue
            ...
            if skippedCount > 0 {
                print(
                    "Filtered out \(skippedCount) invalid records from \(bridgeRecords.count) total"
                )
            }
        #endif
    ...
    func generateRoutes(from bridges: [BridgeStatusModel]) -> [RouteModel] {
        ...
        if let cachedRoutes: [RouteModel] = loadFromCache(
            [RouteModel].self,
