...
public final class BasicTrafficProfileProvider: TrafficProfileProvider {
  ...
  public func getTrafficMultiplier(for date: Date,
                                   segmentType: RoadSegmentType) -> Double
  {
  ...
  public func getActiveProfile() -> TrafficProfile? {
    do {
      let descriptor = FetchDescriptor<TrafficProfile>(predicate: #Predicate<TrafficProfile> { profile in
  ...
  public func createProfile(name: String, profileDescription: String = "")
    -> TrafficProfile
  {
    let profile = TrafficProfile(name: name,
  ...
  private func getTimeMultiplier(for timePeriod: TimePeriod,
                                 profile: TrafficProfile) -> Double
  {
    switch timePeriod {
    case .morningRush:
  ...
  private func setupDefaultProfileIfNeeded() {
    ...
    if profiles.isEmpty {
      ...
      let defaultProfile = TrafficProfile(name: "Seattle Default",
                                          profileDescription:
