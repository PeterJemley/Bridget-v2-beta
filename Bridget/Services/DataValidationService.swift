...
public class DataValidationService {
        ...
        -> DataValidationResult
    {
        ...
        result.recordsPerBridge = Dictionary(
            grouping: ticks,
            by: { $0.bridge_id }
        )
        .mapValues { $0.count }
        ...
        -> DataValidationResult
    {
        ...
        // Create local, method-scoped formatters (thread-safe usage)
        ...
        // Parse timestamps with sanitization and local formatters
        for (index, tick) in ticks.enumerated() {
            if let sanitizedTimestamp = sanitizeTimestamp(tick.ts_utc),
                let date = isoWithFractional.date(from: sanitizedTimestamp)
                    ?? isoBasic.date(from: sanitizedTimestamp)
            {
        ...
        -> DataValidationResult
    {
        ...
        // Local, method-scoped formatters
        let isoWithFractional = ISO8601DateFormatter()
        isoWithFractional.formatOptions = [
            .withInternetDateTime, .withFractionalSeconds, .withTimeZone,
        ]
        ...
        let timestamps = ticks.compactMap { tick -> Date? in
            ...
            return isoWithFractional.date(from: sanitized)
                ?? isoBasic.date(from: sanitized)
        }.sorted()
        ...
        -> DataValidationResult
    {
        ...
        let hourGroups = Dictionary(grouping: ticks) { tick in
            if let sanitized = sanitizeTimestamp(tick.ts_utc),
                let date = isoWithFractional.date(from: sanitized)
                    ?? isoBasic.date(from: sanitized)
            {
