...
public enum CoordinateSystem: String, CaseIterable {
    case wgs84 = "WGS84"
    case nad83 = "NAD83"
    case nad27 = "NAD27"
...
public struct TransformationMatrix: Codable, Equatable {
    ...
    public static func translation(latOffset: Double, lonOffset: Double) -> TransformationMatrix {
        TransformationMatrix(latOffset: latOffset, lonOffset: lonOffset)
    }
    
    /// Creates a scale-only transformation
    public static func scale(latScale: Double, lonScale: Double) -> TransformationMatrix {
...
public struct BridgeTransformation: Codable, Equatable {
...
public struct TransformationResult {
    ...
    public static func failure(_ error: TransformationError) -> TransformationResult {
...
public protocol CoordinateTransformService {
    ...
    func transformToReferenceSystem(
        latitude: Double,
        longitude: Double,
        from sourceSystem: CoordinateSystem,
        bridgeId: String?
...
public final class DefaultCoordinateTransformService: CoordinateTransformService {
    ...
    public init(
        ...
        let finalTransformations = bridgeTransformations.isEmpty ? 
    ...
    public func transform(
        ...
        guard isValidCoordinate(latitude: latitude, longitude: longitude) else {
            return .failure(.invalidInputCoordinates(latitude: latitude, longitude: longitude))
        }
        
        // Check if transformation is supported
        guard canTransform(from: sourceSystem, to: targetSystem, bridgeId: bridgeId) else {
            return .failure(.unsupportedCoordinateSystem(sourceSystem))
        }
        
        // Get transformation matrix
        guard let matrix = calculateTransformationMatrix(
            from: sourceSystem,
            to: targetSystem,
            bridgeId: bridgeId
        ) else {
        ...
        do {
            ...
            if enableLogging {
                print("📍 CoordinateTransformService: Transformed (\(latitude), \(longitude)) to (\(transformedLat), \(transformedLon))")
            }
    ...
    public func canTransform(
        ...
        if (sourceSystem == .seattleAPI && targetSystem == .seattleReference) ||
           (sourceSystem == .seattleReference && targetSystem == .seattleAPI) {
            return true
        }
    ...
    private func getBridgeSpecificMatrix(bridgeId: String?) -> TransformationMatrix? {
        guard let bridgeId = bridgeId else {
            return defaultTransformationMatrix
        }
        
        return bridgeTransformations[bridgeId]?.transformationMatrix ?? defaultTransformationMatrix
    }
    ...
    private func applyTransformation(
        ...
        guard isValidCoordinate(latitude: transformedLat, longitude: transformedLon) else {
    ...
    private static func createDefaultTransformations() -> [String: BridgeTransformation] {
...
extension DefaultCoordinateTransformService {
    ...
    public static let shared = DefaultCoordinateTransformService(enableLogging: true)
}
