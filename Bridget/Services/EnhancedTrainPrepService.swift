...
public class EnhancedTrainPrepService {
    ...
    private func executeFeatureEngineering() async throws {
        ...
        if configuration.enableParallelization {
            try await executeParallelFeatureEngineering(
                featureService,
                ticks: sampleTicks
            )
        } else {
            try await executeSerialFeatureEngineering(
                featureService,
                ticks: sampleTicks
            )
        }
    ...
    private func executeParallelFeatureEngineering(
        _ featureService: FeatureEngineeringService,
        ticks: [ProbeTickRaw]
    ) async throws {
        logger.info(
    ...
    private func createStageCheckpoint(_ stage: PipelineStage) async throws {
        ...
        await progressDelegate?.pipelineDidCreateCheckpoint(
            stage,
            at: checkpointPath
        )
    }
    ...
    @MainActor
    private func updateStageMetrics(
...
extension RecoveryService {
    ...
    func loadCheckpoint(stage: PipelineStage) throws -> PipelineExecutionState {
        guard
            let state = try loadCheckpoint(
                PipelineExecutionState.self,
                for: stage,
                id: "pipeline_state"
            )
        else {
