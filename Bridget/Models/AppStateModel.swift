...
class AppStateModel {
    ...
    /// Prevents duplicate initial loads if startInitialLoad() is called multiple times.
    @ObservationIgnored
    private var hasStartedInitialLoad: Bool = false

    /// Initializes the application state model with default values and the provided persistence.
    ///
    /// - Parameters:
    ...
    ///
    /// - Note: Does not start any asynchronous work during initialization to avoid
    ///         capturing `self` in an escaping closure before initialization completes.
    init(
        ...
        // Intentionally do not start Task{} here. Call startInitialLoad() after init.
    }
    ...
    @MainActor convenience init() {
            ...
            do {
                ...
                // Intentionally do not start Task{} here. Tests can call startInitialLoad() when needed.
            } catch {
                fatalError(
    ...
    /// Starts the initial asynchronous data load.
    ///
    /// Call this after creating an `AppStateModel` instance, e.g., from a SwiftUI `.task` modifier
    /// or immediately after initialization by the composition root.
