...
final class PipelineMetricsViewModel {
    ...
    func toggleProblematicFilter() {
        stageFilter = (stageFilter == .problematicOnly) ? .none : .problematicOnly
    }

    func clearFilters() { stageFilter = .none }

    func filteredStages(from data: PipelineMetricsData) -> [PipelineStageMetric]? {
        switch stageFilter {
        case .none: return nil
        case .problematicOnly:
            let threshold = validationWarningThreshold
            return data.stageMetrics.filter { $0.errorCount > 0 || $0.validationRate < threshold }
        }
    }
