      ...
      ScrollView {
        VStack(spacing: 16) {
          ...
          if viewModel.isLoading {
            ...
            StageDetailsSectionCollapsible(data: data,
            ...
            if let custom = data.customValidationResults,
               !custom.isEmpty
            {
              CustomValidationSectionCollapsible(results: custom,
                                                 isExpanded: $viewModel
                                                   .isCustomValidationExpanded)
            }

            if let stats = data.statisticalMetrics {
              StatisticalUncertaintySectionCollapsible(metrics: stats,
