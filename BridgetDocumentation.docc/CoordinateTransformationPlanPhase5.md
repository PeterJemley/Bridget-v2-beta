...
### Step 4. Batch processing (Day 3)
- [ ] Implement `Sources/CoordinateTransformService+Batch.swift`:
  - Group by `(source, target, bridgeId?)`.
  - Fetch matrix once.
  - Chunk into ~1,024 points per batch.
  - Reuse scratch buffers.
- [ ] Use `withTaskGroup` with concurrency cap = min(physicalCores, 4).
- [ ] Small inputs (`n < 32`) fallback to SIMD.

**Gate E:** ≥2× throughput at 10k points vs scalar baseline.

---

### Step 5. Prewarm & cold-start resiliency (Day 4)
- [ ] Add `Sources/Prewarm.swift`.
- [ ] Implement `prewarm(atStartup:)` for top-N matrices.
- [ ] Optional disk persistence (`transform.cache.disk.enabled`): serialize matrices to JSON/SQLite.

**Gate F:** Cold start → prewarm avoids miss spikes.
...
- `BridgetDocumentation.docc/BenchmarkingGuide.docc` ✅
